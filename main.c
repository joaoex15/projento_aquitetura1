...
void classificacao(struct projeto *proj, int index) {
    ...
    if (strncmp(bin_opcodeT, "00000000000000000000000000000000", 32) == 0) {
        ...
        if (strncmp(bin_opcode, "000000", 6) == 0) {
            int x = proj->bins[index][6] - '0';
            int z = proj->bins[index][7] - '0';
            snprintf(proj->clas[index], sizeof(proj->clas[index]), "mov R[%d] = R[%d]", x, z);
        }
        else if (strncmp(bin_opcode, "000001", 6) == 0) {
            int x = proj->bins[index][6] - '0';
            int z = proj->bins[index][7] - '0';
            snprintf(proj->clas[index], sizeof(proj->clas[index]), "movs R[%d] = R[%d]", x, z);
        }
            ...
            int x = proj->bins[index][6] - '0';
            int z = proj->bins[index][7] - '0';
            snprintf(proj->clas[index], sizeof(proj->clas[index]), "add R[%d] = R[%d]", x, z);
        }
            ...
            int x = proj->bins[index][6] - '0';
            int z = proj->bins[index][7] - '0';
            snprintf(proj->clas[index], sizeof(proj->clas[index]), "l8 R[%d] = MEM8[%d]", x, z);
        } else if (strncmp(bin_opcode, "011010", 6) == 0) {
            int x = proj->bins[index][6] - '0';
            int z = proj->bins[index][7] - '0';
            snprintf(proj->clas[index], sizeof(proj->clas[index]), "l32 R[%d] = MEM32[%d]", x, z);
        } else if (strncmp(bin_opcode, "110111", 6) == 0) {
            int x = proj->bins[index][6] - '0';
            snprintf(proj->clas[index], sizeof(proj->clas[index]), "bun %d", x);
...
int main() {
    ...
    hex = fopen("/home/joao/Área de Trabalho/projentos-arquitetura/1_erro.hex", "r");
    if (hex == NULL) {
